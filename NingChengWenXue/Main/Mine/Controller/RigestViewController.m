...
@implementation RigestViewController
...
-(void) setImageUI {
    ...
    NSArray *imgArray = @[@"账号",@"手机",@"验证码",@"密码",@"密码"];
    for (int i = 0; i<5; i++) {
...
- (void) setAllTextField {
    ...
    // 确认密码
    self.surePassWordField = [[UITextField alloc] initWithFrame:CGRectMake(55, 214, BXScreenW - 70, 52)];
    self.surePassWordField.placeholder = @"请输入确认密码";
    self.surePassWordField.textColor = The_Prompt_Color_Nine;
...
-(void) setRigestButton {
    UIButton *resetBtn = [[UIButton alloc] initWithFrame:CGRectMake(40, 380, BXScreenW - 80, 50)];
    resetBtn.backgroundColor = BXColor(236, 105, 65);
    [resetBtn setTitle:@"注册" forState:UIControlStateNormal];
    [self.rootScrollView addSubview:resetBtn];
    resetBtn.layer.cornerRadius = 10;
    [resetBtn addTarget:self action:@selector(clickRigestButton) forControlEvents:UIControlEventTouchUpInside];
}
...
-(void) clickRigestButton {
    ...
    [weakSelf.helper registWithAccount:self.accountField.text Phone:weakSelf.phonenumField.text Code:weakSelf.codeField.text Password:weakSelf.passWordField.text success:^(NSDictionary *response) {
