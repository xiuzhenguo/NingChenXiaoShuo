...
@interface HEssenceViewController ()<UITableViewDelegate, UITableViewDataSource>;
...
@implementation HEssenceViewController
...
#pragma mark - 超精华网络数据的获取
-(void) getChaojinghuaData {
    ...
    [self.helper chaojinghuaListWithUserId:userId PageIndex:[NSString stringWithFormat:@"%ld",self.pagenum] success:^(NSArray *response) {
        st_dispatch_async_main(^{
            if (self.pagenum == 1) {
