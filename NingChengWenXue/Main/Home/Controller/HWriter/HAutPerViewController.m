...
@interface HAutPerViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation HAutPerViewController
...
-(void) getFansListData {
    ...
    [self.helper fansWithUserId:userId AuthorId:self.autherId PageIndex:[NSString stringWithFormat:@"%ld",self.pagenum] success:^(NSArray *response) {
        st_dispatch_async_main(^{
            if (self.pagenum == 1) {
